...
const loadImage = (name) =>
  new Promise((resolve) => {
    ...
    image.src = `http://localhost:8080/HRMDOtracking/assets/virtualphotobooth/backgrounds/${name}.jpg`;
...
let images = {};
Promise.all([
  loadImage('employeesday'),
]).then(([employeesday]) => {
  images.employeesday = employeesday;
  return images;
});
...
virtualBackgroundButton.onclick = async event => {
  event.preventDefault();
  const options = {maskBlurRadius:5, backgroundImage:"employeesday", fitType:"Fill"};
